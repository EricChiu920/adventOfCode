...
const twoSum = (nums: number[], target: number = 2020): [number, number] | null => {
...
const threeSum = (nums: number[], target: number = 2020): [number, number, number] | null => {
  for (let i = 0; i < nums.length; i += 1) {
    const num = nums[i];
...
const day01 = (): number => {
  const threeSumResult = threeSum(expenseReport);

  if (threeSumResult === null) { return -1; }
  return multiply(threeSumResult);
};
